...
export class AppComponent {
    ...
    if (refresh && this.orgContextType) {
      ...
      if (!this.menuResult) {
        ...
        this.accountService.getMenuFeatures(featureMenuObj).subscribe((result: any) => {
          ...
          this.processMenu();
          // this.getMenu(this.menuResult, 'orgRoleChange', this.accountRoleLevel);
          // this.timeLeft = Number.parseInt(localStorage.getItem("liveFleetTimer"));
  ...
  getMenu(data: any, from?: any, accountRoleLevel?: string) {
    ...
    if (this.orgContextType && !this.isAllOrgData && this.isRoleChanged) {
      this.isRoleChanged = false;
      this.getTranslationLabels(true);
    }
    this.setPageTitle();
  }
  ...
  getTranslationLabels(isRoleChange?: boolean) {
    ...
    if (curAccId) { //- checked for refresh page
      ...
      this.translationService.getLanguageCodes().subscribe(languageCodes => {
        ...
        if(!this.menuResult){
          this.accountService.getMenuFeatures(featureMenuObj).subscribe((result: any) => {
            this.menuResult = result;
            let contextOrgUser = this.menuResult.features.find(item => item.name.indexOf("Admin#Organization-Scope") !== -1);
            if (contextOrgUser) {
  ...
  getAllOrg(preferencelanguageCode: any,isRoleChange?: boolean){
    this.organizationService.getAllOrganizations().subscribe((data: any) => {
      if (data) {
        ...
        if (!isRoleChange)
          this.calledTranslationLabels(preferencelanguageCode);
        this.isAllOrgData = true;
      }
    }, (error) => {
      //TODO: seggregate 4xx & 5xx error code handling
