...
// }
  ...
  tripCheckboxClicked(event: any, row: any) {
    ...
    let _ui: any = this.mapService.getUI();
    if(event.checked){
      this.rowdata.push(row);
...
setVehicleGroupAndVehiclePreSelection() {
  ...
  exportAsExcelFile() {
    ...
    const title = this.translationData.lblFleetFuelTripReport;
  ...
  pdfColumns.push(pdfColumnHeads);  
    let prepare = []
      ...
      var pdfName = this.translationData.lblFleetFuelPdfTitle;
      html2canvas( DATA)
      .then(canvas => {  
