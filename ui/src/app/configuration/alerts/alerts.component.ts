...
export class AlertsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.translationService
      .getPreferences(this.localStLanguage)
      .subscribe((res: any) => {
        if (
          this.accountPrefObj.accountPreference &&
          this.accountPrefObj.accountPreference != ''
        ) {
          // account pref
          ...
          this.organizationService
            .getOrganizationPreference(this.accountOrganizationId)
            .subscribe(
              (orgPref: any) => {
  ...
  proceedStep(_prefData: any, preference: any) {
    ...
    let _search = _prefData?.timeformat.filter(
      (i) => i.id == preference.timeFormatId
    );
    this.prefTimeFormat = Number(_search[0].name.split('_')[1].substring(0, 2));
    this.prefTimeZone = _prefData?.timezone.filter(
      (i) => i.id == preference.timezoneId
    )[0].name;
    this.prefDateFormat = _prefData?.dateformat.filter(
      (i) => i.id == preference.dateFormatTypeId
    )[0].name;
    this.prefUnitFormat = _prefData?.unit.filter(
      (i) => i.id == preference.unitId
    )[0].name;
    let vehicleDisplayId = preference.vehicleDisplayId;
    if (vehicleDisplayId) {
      let vehicledisplay = _prefData?.vehicledisplay.filter(
        (el) => el.id == vehicleDisplayId
      );
  ...
  loadFiltersData() {
    this.showLoadingIndicator = true;
    this.alertService
      .getAlertFilterData(this.accountId, this.accountOrganizationId)
      .subscribe(
        (data) => {
          ...
          this.alertCategoryList = filterData.filter(
            (item) => item.type == 'C'
          );
          this.alertTypeList = filterData.filter((item) => item.type == 'T');
          this.alertCriticalityList = filterData.filter(
            (item) => item.type == 'U'
          );
          this.vehicleList = data['vehicleGroup'].filter(
            (item) => item.vehicleName != ''
          );
          this.vehicleList = this.removeDuplicates(
            this.vehicleList,
            'vehicleName'
          );
  ...
  loadDataBasedOnPrivileges() {
    this.showLoadingIndicator = true;
    this.alertService
      .getAlertFilterDataBasedOnPrivileges(this.accountId, this.accountRoleId)
      .subscribe((data) => {
        ...
        this.alertCriticalityList = this.alertCategoryTypeMasterData.filter(
          (item) => item.type == 'U'
        );
        ...
        if (this.alertTypeList.length != 0) {
          ...
          this.alertCategoryList = this.getUnique(
        ...
        this.associatedVehicleData.forEach((element) => {
          ...
          if (element.vehicleId && element.vehicleId != '') {
            this.vehicleListBasedOnPrivilege.push({
              vehicleId: parseInt(element.vehicleId),
            });
          }
        });

        this.vehicleGroupListBasedOnPrivilege = this.removeDuplicates(
          this.vehicleGroupListBasedOnPrivilege,
          'vehicleGroupId'
        );
        this.finalVehicleGroupList =
          this.vehicleGroupListBasedOnPrivilege.filter(
            (i) => i.vehicleGroupId >= 0
          ); // NaN removed

        this.vehicleByVehGroupList = this.getUniqueVINs([
          ...this.associatedVehicleData,
          ...this.singleVehicle,
        ]);
        this.alertStatusList = [
          {
  ...
  loadAlertsData() {
    ...
    this.showLoadingIndicator = true;
    this.alertService
      .getAlertData(this.accountId, this.accountOrganizationId)
      .subscribe(
        (data) => {
          this.hideloader();
          ...
          initDataBasedOnPrivilege.forEach((item) => {
            let hasPrivilege =
              this.alertTypeListBasedOnPrivilege.filter(
                (element) => element.enum == item.type
              ).length > 0 &&
              (this.vehicleGroupListBasedOnPrivilege.filter(
                (element) => element.vehicleGroupId == item.vehicleGroupId
              ).length > 0 ||
                this.vehicleListBasedOnPrivilege.filter(
                  (element) => element.vehicleId == item.vehicleGroupId
                ).length > 0);
            if (hasPrivilege) {
          ...
          this.initData.forEach((item) => {
            ...
            let catVal = this.alertCategoryList.filter(
              (cat) => cat.enum == item.category
            );
            catVal.forEach((obj) => {
              item['category'] = obj.value;
            });
            let typeVal = this.alertTypeList.filter(
              (type) => type.enum == item.type
            );
            typeVal.forEach((obj) => {
            ...
            let critical = item.alertUrgencyLevelRefs.filter(
              (lvl) => lvl.urgencyLevelType == 'C'
            );
            let warning = item.alertUrgencyLevelRefs.filter(
              (lvl) => lvl.urgencyLevelType == 'W'
            );
            let advisory = item.alertUrgencyLevelRefs.filter(
              (lvl) => lvl.urgencyLevelType == 'A'
            );

            if (critical.length > 0) {
              critical.forEach((obj) => {
                item = Object.defineProperty(item, 'highUrgencyLevel', {
                  value: obj.urgencyLevelType,
                  writable: true,
                  enumerable: true,
                  configurable: true,
                });
                item = Object.defineProperty(item, 'highThresholdValue', {
                ...
                if (obj.unitType != 'N') {
                  item.highThresholdValue = this.getConvertedThresholdValues(
                    item.highThresholdValue,
                    obj.unitType
                  );
                }
              });
            ...
            if (item.vehicleGroupName != '') {
              this.vehicleGroupList.push({
                value: item.vehicleGroupName,
                key: item.vehicleGroupName,
              });
              this.vehicleGroupList = this.vehicleGroupList.filter(
                (test, index, array) =>
                  index ===
                  array.findIndex((findTest) => findTest.value === test.value)
              );
            } else {
              //according to pref
              let vehicleDisplayId =
                this.accountPrefObj.accountPreference.vehicleDisplayId;
              if (vehicleDisplayId) {
                let vehicledisplay = this.prefData?.vehicledisplay.filter(
                  (el) => el.id == vehicleDisplayId
                );
                if (vehicledisplay.length != 0) {
                  this.vehicleDisplayPreference = vehicledisplay[0].name;
              ...
              if (
                this.vehicleDisplayPreference == 'dvehicledisplay_VehicleName'
              ) {
                item.vehicleGroupName = item.vehicleName;
              } else if (
                this.vehicleDisplayPreference ==
                'dvehicledisplay_VehicleIdentificationNumber'
              ) {
  ...
  getConvertedThresholdValues(originalThreshold, unitType) {
    ...
    if (unitType == 'H' || unitType == 'T' || unitType == 'S') {
      ...
      threshold = this.reportMapService.getConvertedDistance(
        originalThreshold,
        unitType
      );
  ...
  setUnitOfThreshold(item) {
    ...
    switch (item.category + item.type) {
      ...
      case 'LU':
        unitTypeEnum = 'H';
        unitType = item.alertUrgencyLevelRefs
          ? item.alertUrgencyLevelRefs[0].unitType
          : 'S';
        if (unitType == 'H') {
          item.UnitTypeVal = this.translationData.lblHours;
        } else if (unitType == 'T') {
  ...
  getNewTagData(data: any) {
    ...
    newTrueData.sort(
      (userobj1, userobj2) =>
        parseInt(userobj2.createdAt) - parseInt(userobj1.createdAt)
    );
  ...
  getDeletMsg(alertName: any, isError?: boolean) {
    if (!isError) {
      ...
      else return "Alert '$' was successfully deleted".replace('$', alertName);
    } else {
      if (this.translationData.lblAlertDeleteError)
        return this.translationData.lblAlertDeleteError.replace('$', alertName);
      else
        return "Alert '$' cannot be deleted as notification is associated with this".replace(
          '$',
          alertName
        );
    }
  ...
  onViewAlert(row: any, action: any) {
    ...
    this.rowsData = this.originalAlertData.filter(
  ...
  onChangeAlertStatus(rowData: any) {
    const options = {
      ...
      message:
        rowData.state == 'A'
          ? this.translationData.lblYouwanttoDeActivate
          : this.translationData.lblYouwanttoActivate,
      cancelText: this.translationData.lblCancel,
      confirmText:
        rowData.state == 'A'
          ? this.translationData.lblDeactivate
          : this.translationData.lblActivate,
    ...
    this.dialogRef = this.dialog.open(
      ActiveInactiveDailogComponent,
      dialogConfig
    );
    this.dialogRef.afterClosed().subscribe((res: any) => {
      if (res == true) {
        if (rowData.state == 'A') {
          this.alertService.suspendAlert(rowData.id).subscribe(
            (data) => {
              this.loadAlertsData();
            },
            (error) => {
          ...
          this.alertService.activateAlert(rowData.id).subscribe(
            (data) => {
              this.loadAlertsData();
            },
            (error) => {
  ...
  onVehicleGroupClick(data: any) {
    ...
    const colsName = [
      this.translationData.lblVehicleName,
      this.translationData.lblVIN,
      this.translationData.lblRegistrationNumber,
      this.translationData.lblStatus,
    ];
    ...
    this.showLoadingIndicator = true;
    this.vehicleService.getVehiclesDetails(objData).subscribe(
      (vehList: any) => {
        this.hideloader();
  ...
  addSubscribrNonSubscribeStatus(_vehList: any, rowData: any) {
    if (rowData && rowData.keyType && rowData.keyType != '') {
      let featuresData: any = this.alertCategoryTypeFilterData.filter(
        (i) => i.featureKey == rowData.keyType
      );
          ...
          if (_orgSub && _orgSub.length > 0) {
            ...
            _vehList.forEach((_el) => {
              let _find: any = featuresData.filter(
                (j) => j.vehicleId == _el.id && j.subscriptionType == 'V'
              );
