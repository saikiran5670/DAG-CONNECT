...
export class CreateEditViewAlertsComponent implements OnInit {
  ...
  displayedColumnsGeofence: string[] = [
    'select',
    'name',
    'categoryName',
    'subCategoryName',
  ...
  displayedColumnsCorridor: string[] = [
    'select',
    'corridoreName',
    'startPoint',
    'endPoint',
  ...
  public filteredVehicleGroups: ReplaySubject<String[]> = new ReplaySubject<
    String[]
  >(1);

  public filteredVehicles: ReplaySubject<String[]> = new ReplaySubject<
    String[]
  >(1);
  ...
  ngOnInit() {
    if (localStorage.getItem('contextOrgId')) {
      this.accountOrganizationId = localStorage.getItem('contextOrgId')
        ? parseInt(localStorage.getItem('contextOrgId'))
        : 0;
    } else {
      this.accountOrganizationId = localStorage.getItem('accountOrganizationId')
        ? parseInt(localStorage.getItem('accountOrganizationId'))
        : 0;
    }
    let today = new Date();
    ...
    this.accountId = localStorage.getItem('accountId')
      ...
      : 0;
    this.accountRoleId = localStorage.getItem('accountRoleId')
      ? parseInt(localStorage.getItem('accountRoleId'))
      : 0;
    ...
    this.alertForm = this._formBuilder.group(
      ...
      {
        validator: [CustomValidators.specialCharValidationForName('alertName')],
      }
    );
    ...
    if (this.finalVehicleGroupList && this.finalVehicleGroupList.length > 0) {
      ...
      let _vehGrpList: any = {
        vehicleGroupIds: this.finalVehicleGroupList.map(
          (i) => i.vehicleGroupId
        ),
      };
      this.alertService
        .getSubscribeNonSubsucribeVehicles(_vehGrpList)
        .subscribe(
          (vehicleGrpList: any) => {
            if (
              vehicleGrpList &&
              vehicleGrpList.vehicleandGroupDetails &&
              vehicleGrpList.vehicleandGroupDetails.length > 0
            ) {
              this.vehicleGrpByVehicleList =
                vehicleGrpList.vehicleandGroupDetails.slice();
            }
            this.stepForword();
    ...
    if (this.actionType == 'edit' || this.actionType == 'duplicate') {
      ...
      this.alertCategoryName =
        this.translationData[
          this.alertCategoryTypeMasterData.filter(
            (item) => item.enum == this.alert_category_selected
          )[0].key
        ];
      this.alertTypeName =
        this.translationData[
          this.alertCategoryTypeMasterData.filter(
            (item) =>
              item.enum == this.selectedRowData.type &&
              item.parentEnum == this.alert_category_selected
          )[0].key
        ];
      this.onChangeAlertType(this.selectedRowData.type);
      if (this.selectedRowData.alertUrgencyLevelRefs[0].periodType == 'C') {
        this.from = this.formStartDate(
          new Date(
            this.selectedRowData.alertUrgencyLevelRefs[0].urgencylevelStartDate
          ),
          this.prefTimeFormat,
          this.prefDateFormat
        ).split(' ');
        this.to = this.formStartDate(
          new Date(
            this.selectedRowData.alertUrgencyLevelRefs[0].urgencylevelEndDate
          ),
          this.prefTimeFormat,
          this.prefDateFormat
        ).split(' ');
  ...
  onChangeAlertCategory(value) {
    ...
    this.alertTypeByCategoryList = this.alertTypeList.filter(
      (item) => item.parentEnum == value
    );
    this.alertTypeByCategoryList = this.alertTypeByCategoryList.filter(
      (item) => item.enum != 'Y'
    );
  }
  ...
  onChangeAlertType(value: any, flag?: boolean) {
    ...
    let alertTypeObj = this.alertCategoryTypeMasterData.filter(
      (item) =>
        item.enum == this.alert_type_selected &&
        item.parentEnum == this.alert_category_selected
    )[0];
    this.alertTypeObject = alertTypeObj;
    if (this.actionType != 'view') {
      this.alertTypeName = this.alertTypeList.filter(
        (item) => item.enum == this.alert_type_selected
      )[0].value;
    ...
    if (flag) {
      // default selection after alert type change
      this.alertForm.get('vehicleGroup').setValue('ALL');
      this.onChangeVehicleGroup('ALL');
    }

    if (
      this.alert_category_selected === 'L' &&
      (this.alert_type_selected === 'N' ||
        this.alert_type_selected === 'X' ||
        this.alert_type_selected === 'C' ||
        this.alert_type_selected === 'S')
    ) {
      if (this.actionType == 'edit' || this.actionType == 'duplicate') {
        this.alertForm
          .get('alertLevel')
          .setValue(
            this.selectedRowData.alertUrgencyLevelRefs[0].urgencyLevelType
          );
      }
      this.loadMap();
      if (
      ...
      if (this.alert_type_selected === 'O') {
        // Status Change to Stop Now
        this.alertForm.get('alertLevel').setValue('C');
      } else if (this.alert_type_selected === 'E') {
        // Status Change to Service Now
        this.alertForm.get('alertLevel').setValue('W');
      }
    } else if (
      (this.alert_category_selected == 'L' &&
        (this.alert_type_selected == 'Y' ||
          this.alert_type_selected == 'H' ||
          this.alert_type_selected == 'D' ||
          this.alert_type_selected == 'U' ||
          this.alert_type_selected == 'G')) ||
      (this.alert_category_selected == 'F' &&
        (this.alert_type_selected == 'P' ||
          this.alert_type_selected == 'L' ||
          this.alert_type_selected == 'T' ||
          this.alert_type_selected == 'I' ||
          this.alert_type_selected == 'A' ||
          this.alert_type_selected == 'F'))
    ) {
      if (
        this.actionType == 'edit' ||
      ...
      if (
        this.alert_category_selected + this.alert_type_selected == 'LU' ||
        this.alert_category_selected + this.alert_type_selected == 'FI'
      ) {
        this.unitTypes = [
          {
            enum: 'H',
            value: this.translationData.lblHours
              ? this.translationData.lblHours
              : '',
          },
          {
            enum: 'T',
            value: this.translationData.lblMinutes
              ? this.translationData.lblMinutes
              : '',
          },
          {
            enum: 'S',
            value: this.translationData.lblSeconds
              ? this.translationData.lblSeconds
              : '',
          },
        ];
      }
      ...
      switch (this.alert_category_selected + this.alert_type_selected) {
        case 'LH': {
          ...
          this.labelForThreshold = this.translationData.lblPeriod
            ? this.translationData.lblPeriod
            : '';
          this.unitForThreshold = this.translationData.lblHours
            ? this.translationData.lblHours
            : '';
          this.unitTypeEnum = 'H';
          break;
        ...
        case 'LD': {
          //Excessive distance done
          this.labelForThreshold = this.translationData.lblDistance
            ? this.translationData.lblDistance
            : '';
          this.unitForThreshold =
            this.prefUnitFormat == 'dunit_Metric'
              ? this.translationData.lblkm
              : this.translationData.lblmile;
          ...
          if (
            ...
            this.alertForm
              .get('unitType')
              .setValue(this.selectedRowData.alertUrgencyLevelRefs[0].unitType);
          } else {
        ...
        case 'LU': {
          //Excessive Driving duration
          this.labelForThreshold = this.translationData.lblDuration
            ? this.translationData.lblDuration
            : '';
          this.unitForThreshold = this.translationData.lblHours
            ? this.translationData.lblHours
            : '';
          ...
          if (
            ...
            this.onChangeUnitType(
              this.selectedRowData.alertUrgencyLevelRefs[0].unitType
            );
          } else {
            this.alertForm.get('unitType').setValue(this.unitTypeEnum);
          }
        ...
        case 'LG': {
          //Excessive Global Mileage
          this.labelForThreshold = this.translationData.lblMileage
            ? this.translationData.lblMileage
            : '';
          this.unitForThreshold =
            this.prefUnitFormat == 'dunit_Metric'
              ? this.translationData.lblkm
        ...
        case 'FP': {
          //Fuel Increase During stop
          this.labelForThreshold = this.translationData.lblPercentage
            ? this.translationData.lblPercentage
            : '';
          this.unitForThreshold = '%';
          this.unitTypeEnum = 'P';
          break;
        ...
        case 'FI': {
          //Excessive Average Idling
          this.labelForThreshold = this.translationData.lblDuration
            ? this.translationData.lblDuration
            : '';
          this.unitForThreshold = this.translationData.lblSeconds
            ? this.translationData.lblSeconds
            : '';
          this.unitTypeEnum = 'S';
          if (
        ...
        case 'FA': {
          ...
          this.labelForThreshold = this.translationData.lblSpeed
            ? this.translationData.lblSpeed
            : '';
          this.unitForThreshold =
            this.prefUnitFormat == 'dunit_Metric'
              ? this.translationData.lblkilometerperhour
              : this.translationData.lblMilesPerHour;
          if (this.prefUnitFormat == 'dunit_Metric') {
            this.unitTypeEnum = 'A';
          } else {
            this.unitTypeEnum = 'B';
        ...
        case 'FF': {
          //Fuel Consumed
          this.labelForThreshold = this.translationData.lblFuelConsumed
            ? this.translationData.lblFuelConsumed
            : '';
          this.unitForThreshold = '%';
          this.unitTypeEnum = 'P';
          break;
  ...
  updateVehiclesList(alertTypeObj: any) {
    this.vehicleByVehGroupList = [];
    this.vehicleListForTable = [];
    let vehicles = this.getUnique(
      this.alertCategoryTypeFilterData.filter(
        (item) => item.featureKey == alertTypeObj.key
      ),
      'vehicleId'
    );
    vehicles.forEach((element) => {
      let veh = this.associatedVehicleData.filter(
        (item) =>
          item.vehicleId == element.vehicleId &&
          item.vehicleGroupId == this.selectedRowData.vehicleGroupId
      );
      if (veh.length > 0) {
    ...
    this.getUnique(this.associatedVehicleData, 'vehicleId').forEach(
      (element) => {
  ...
  getVehicleGroupsForAlertType(alertTypeObj: any) {
    ...
    this.vehicleGroupList = this.getUnique(
      this.vehicleGroupList,
      'vehicleGroupId'
    );
  ...
  getVehiclesForAlertType(alertTypeObj: any) {
    ...
    let featuresData = this.alertCategoryTypeFilterData.filter(
      (item) => item.featureKey == alertTypeObj.key
    );
    if (featuresData.length == 1 && featuresData[0].subscriptionType == 'O') {
      // org based - all subscribe
      ...
      if (featuresData.length > 0) {
        ...
        let _orgSub: any = featuresData.filter(
          (i) => i.subscriptionType == 'O'
        ); // find org based subscription
        if (_orgSub && _orgSub.length > 0) {
          // all vehicle subscribe
          ...
          this.vehicleByVehGroupList.forEach((_el) => {
            let _find: any = featuresData.filter(
              (j) => j.vehicleId == _el.vehicleId && j.subscriptionType == 'V'
            );
            if (_find && _find.length > 0) {
              // find 'V'
  ...
  filterVehiclesFromResp(grpId?: any) {
    ...
    let grpData: any = [];
    if (grpId) {
      // search by veh-grp-id
      grpData = this.vehicleGrpByVehicleList.filter(
        (i) => parseInt(i.vehicleGroupId) == parseInt(grpId)
      );
    } else {
      // all veh-grp
    ...
    _vehicleConcernList = this.removeDuplicates(
      _vehicleConcernList,
      'vehicleId'
    ); // remove deplicate vehicle
  ...
  getUniqueVINs(vinList: any) {
    ...
    for (let vin of vinList) {
      let vinPresent = uniqueVINList
        .map((element) => element.vin)
        .indexOf(vin.vin);
      if (vinPresent == -1) {
  ...
  onChangeVehicleGroup(value: any) {
    ...
    if (value == 'ALL') {
      this.getVehiclesForAlertType(alertTypeObj);
      this.vehicleByVehGroupList = this.getUniqueVINs([
        ...this.vehicleByVehGroupList,
        ...this.singleVehicle,
      ]);
    } else {
      ...
      if (featuresData.length == 1 && featuresData[0].subscriptionType == 'O') {
        // org based -> all veh subscribe
        this.vehicleByVehGroupList = this.associatedVehicleData.filter((item) =>
          item.vehicleGroupDetails.includes(this.vehicle_group_selected + '~')
        );
        this.vehicleByVehGroupList.forEach((elem) => {
          elem.subscriptionType = 'Subscribed';
        });

        vehicleConcernList = this.filterVehiclesFromResp(
          this.vehicle_group_selected
        ); // #20710
        vehicleConcernList.forEach((element) => {
          element.subscriptionType = 'Subscribed';
        });
        ...
        if (featuresData.length > 0) {
          let vehicle = this.associatedVehicleData.filter((item) =>
            item.vehicleGroupDetails.includes(this.vehicle_group_selected + '~')
          );
          if (vehicle.length > 0) {
  ...
  onChangeVehicle(value) {
    ...
    let vehicleSelected = this.vehicleByVehGroupList.filter(
      (item) => item.vehicleId == value
    );
    ...
    if (this.alert_type_selected == 'C') {
      ...
      setTimeout(() => {
        ...
        var ms = new H.ui.MapSettingsControl({
          baseLayers: [
            {
              label: this.translationData.lblNormal,
              layer: defaultLayers.raster.normal.map,
            },
            {
              label: this.translationData.lblSatellite,
              layer: defaultLayers.raster.satellite.map,
            },
            {
              label: this.translationData.lblTerrain,
              layer: defaultLayers.raster.terrain.map,
            },
          ],
          layers: [
            {
              label: this.translationData.lblLayerTraffic,
              layer: defaultLayers.vector.normal.traffic,
            },
            {
              label: this.translationData.lblLayerIncidents,
  ...
  addMarkerOnMap(ui) {
    ...
    this.markerArray.forEach((element) => {
      ...
      this.createResizableCircle(
        this.alertForm.controls.widthInput.value * 1000,
        this.ui,
        element
      );
  ...
  createResizablePolygon(
    ...
    if (thisRef.actionType == 'create') {
      //-- only for create polygon geofence
      ...
      verticeGroup.addEventListener(
        'dragend',
        function (ev) {
          var coordinate = map.screenToGeo(
            ev.currentPointer.viewportX,
            ev.currentPointer.viewportY
          );
          let nodeIndex = ev.target.getData()['verticeIndex'];
          let _position =
            Math.abs(coordinate.lat.toFixed(4)) +
            ',' +
            Math.abs(coordinate.lng.toFixed(4));
          if (_position) {
            thisRef.hereService.getAddressFromLatLng(_position).then(
              (result) => {
  ...
  addMarkersAndSetViewBoundsGeofence(map, row) {
    ...
    row.nodes.forEach((element) => {
      locationObjArray.push(
        new H.map.Marker({ lat: element.latitude, lng: element.longitude })
      );
    });
  ...
  addPolylineToMap() {
    ...
    let group = new H.map.Group();
    group.addObjects([
      new H.map.Polyline(lineString, { style: { lineWidth: 4 } }),
    ]);
  ...
  setDefaultValue() {
    ...
    if (this.selectedRowData.applyOn == 'G') {
      this.alertForm
        .get('vehicleGroup')
        .setValue(this.selectedRowData.vehicleGroupId);
    ...
    // this.onChangeAlertType(this.selectedRowData.type);
    this.convertValuesToPrefUnit();
    this.selectedApplyOnPeriod =
      this.selectedRowData.alertUrgencyLevelRefs[0].periodType;
    if (this.selectedApplyOnPeriod == 'C') {
      this.from = this.convertToFromTime(
        this.selectedRowData.alertUrgencyLevelRefs[0].urgencylevelStartDate
      );
      this.to = this.convertToFromTime(
        this.selectedRowData.alertUrgencyLevelRefs[0].urgencylevelEndDate
      );
  ...
  convertValuesToPrefUnit() {
    ...
    this.selectedRowData.alertUrgencyLevelRefs.forEach((element) => {
      if (
        this.alert_category_selected + this.alert_type_selected == 'LU' ||
        this.alert_category_selected + this.alert_type_selected == 'LH' ||
        this.alert_category_selected + this.alert_type_selected == 'FI'
      ) {
        threshold = this.reportMapService.getConvertedTime(
      ...
      if (
        this.alert_category_selected + this.alert_type_selected == 'LD' ||
        this.alert_category_selected + this.alert_type_selected == 'LG'
      ) {
        if (this.prefUnitFormat == 'dunit_Metric') {
          this.unitTypeEnum = 'K';
        } else {
        ...
        threshold = this.reportMapService.getDistance(
          element.thresholdValue,
          this.prefUnitFormat
        );
        if (element.urgencyLevelType == 'C') {
      ...
      if (this.alert_category_selected + this.alert_type_selected == 'FA') {
        threshold = this.reportMapService.getConvertedSpeed(
          element.thresholdValue,
          this.unitTypeEnum
        );
        if (element.urgencyLevelType == 'C') {
      ...
      if (
        this.alert_category_selected + this.alert_type_selected == 'FL' ||
        this.alert_category_selected + this.alert_type_selected == 'FT' ||
        this.alert_category_selected + this.alert_type_selected == 'FP' ||
        this.alert_category_selected + this.alert_type_selected == 'FF'
      ) {
        if (element.urgencyLevelType == 'C') {
          this.alertForm
            .get('criticalLevelThreshold')
            .setValue(element.thresholdValue);
        } else if (element.urgencyLevelType == 'W') {
          this.alertForm
            .get('warningLevelThreshold')
            .setValue(element.thresholdValue);
        } else {
          this.alertForm
            .get('advisoryLevelThreshold')
            .setValue(element.thresholdValue);
        }
      }
    });
  ...
  getBreadcum() {
    ...
    if (this.actionType == 'edit')
      page = this.translationData.lblEditAlertDetails
        ? this.translationData.lblEditAlertDetails
        : 'Edit Alert Details';
    else if (this.actionType === 'view')
      page = this.translationData.lblViewAlertDetails
        ? this.translationData.lblViewAlertDetails
        : 'View Alert Details';
    else if (this.actionType === 'create' || this.actionType === 'duplicate')
      page = this.translationData.lblCreateNewAlert
        ? this.translationData.lblCreateNewAlert
        : 'Create New Alert';

    return `${
      this.translationData.lblHome ? this.translationData.lblHome : 'Home'
    } / 
    ${
      this.translationData.lblConfiguration
        ? this.translationData.lblConfiguration
        : 'Configuration'
    } / 
    ${
  ...
  loadPOISelectedData(tableData: any) {
    ...
    if (this.actionType == 'view') {
      tableData.forEach((row: any) => {
        let search = this.selectedRowData.alertLandmarkRefs.filter(
          (item) => item.refId == row.id && item.landmarkType == 'P'
        );
        if (search.length > 0) {
  ...
  selectPOITableRows(rowData: any, event?: any) {
    if (event) {
      this.poiDataSource.data.forEach((row: any) => {
        let search = rowData.landmarks.filter(
          (item) => item.landmarkid == row.id && item.type == 'P'
        );
        if (search.length > 0) {
          if (event.checked) this.selectedPOI.select(row);
          else this.selectedPOI.deselect(row);
          this.PoiCheckboxClicked(event, row);
        }
      });
    } else {
      this.poiDataSource.data.forEach((row: any) => {
        let search = rowData.alertLandmarkRefs.filter(
          (item) => item.refId == row.id && item.landmarkType == 'P'
        );
        if (search.length > 0) {
    ...
    if (this.actionType == 'view') {
      tableData.forEach((row: any) => {
        let search = this.selectedRowData.alertLandmarkRefs.filter(
          (item) =>
            item.refId == row.id &&
            (item.landmarkType == 'C' || item.landmarkType == 'O')
        );
        if (search.length > 0) {
  ...
  selectGeofenceTableRows(rowData: any, event?: any) {
    if (event) {
      this.geofenceDataSource.data.forEach((row: any) => {
        let search = rowData.landmarks.filter(
          (item) =>
            item.landmarkid == row.id && (item.type == 'C' || item.type == 'O')
        );
        if (event && search.length > 0) {
          if (event.checked) this.selectedGeofence.select(row);
          else this.selectedGeofence.deselect(row);
  ...
  loadGroupSelectedData(tableData: any) {
    ...
    if (this.actionType == 'view') {
      tableData.forEach((row: any) => {
        let search = this.selectedRowData.alertLandmarkRefs.filter(
          (item) => item.refId == row.id && item.landmarkType == 'G'
        );
        if (search.length > 0) {
    ...
    if (this.actionType == 'view') {
      tableData.forEach((row: any) => {
        let search = this.selectedRowData.alertLandmarkRefs.filter(
          (item) =>
            item.refId == row.id &&
            (item.landmarkType == 'R' || item.landmarkType == 'E')
        );
        if (search.length > 0) {
  ...
  updatePOIDataSource(tableData: any) {
    ...
    this.poiDataSource.filterPredicate = function (
      data: any,
      filter: string
    ): boolean {
      return (
  ...
  updateGeofenceDataSource(tableData: any) {
    ...
    this.geofenceDataSource.filterPredicate = function (
      data: any,
      filter: string
    ): boolean {
      return (
  ...
  updateGroupDatasource(tableData: any) {
    ...
    this.groupDataSource.filterPredicate = function (
      data: any,
      filter: string
    ): boolean {
      return (
  ...
  updateCorridorDatasource(tableData: any) {
    ...
    this.corridorDataSource.filterPredicate = function (
      data: any,
      filter: string
    ): boolean {
      return (
  ...
  onPOIClick(row: any) {
    ...
    this.landmarkGroupService
      .getLandmarkGroups(objData)
      .subscribe((groupDetails) => {
        this.selectedRowData = groupDetails['groups'][0].landmarks.filter(
          (item) => item.type == 'P'
        );
  ...
  convertThresholdValuesBasedOnUnits() {
    if (this.isCriticalLevelSelected) {
      ...
      if (
        ...
        this.criticalThreshold = this.reportMapService.getTimeInSeconds(
        ...
        this.criticalThreshold =
          this.reportMapService.getConvertedDistanceToMeter(
            this.criticalThreshold,
            this.unitTypeEnum
          );
      } else if (
        this.alert_category_selected + this.alert_type_selected ==
        'FA'
      ) {
        this.criticalThreshold =
          this.reportMapService.getConvertedSpeedToMeterPerSec(
            this.criticalThreshold,
            this.unitTypeEnum
          );
      }
    ...
    if (this.isWarningLevelSelected) {
      ...
      if (
        ...
        this.warningThreshold = this.reportMapService.getTimeInSeconds(
    ...
    if (this.isAdvisoryLevelSelected) {
      ...
      if (
        ...
        this.advisoryThreshold = this.reportMapService.getTimeInSeconds(
  ...
  onCreateUpdate() {
    ...
    if (
      this.alert_category_selected == 'L' &&
      (this.alert_type_selected == 'N' || this.alert_type_selected == 'X')
    ) {
      if (
        this.selectedPOI.selected.length == 0 &&
        this.selectedGeofence.selected.length == 0 &&
        this.selectedGroup.selected.length == 0
      ) {
        const invalidControl = this.el.nativeElement.querySelector(
          '[formcontrolname="' + 'searchForLevelPOI' + '"]'
        );
        if (invalidControl) {
    ...
    if (
      ...
      if (this.selectedCorridor.selected.length == 0) {
        const invalidControl = this.el.nativeElement.querySelector(
          '[formcontrolname="' + 'searchCorridor' + '"]'
        );
        if (invalidControl) {
          invalidControl.scrollIntoView({
            behavior: 'smooth',
    ...
    if (this.isFormValidate) {
      ...
      if (this.panelOpenState) {
        this.notifications =
          this.notificationComponent.getNotificationDetails();
      }
      if (
        this.alert_category_selected == 'L' &&
        this.alert_type_selected === 'S'
        ...
        let alertTimingRefHoursOfService =
          this.periodSelectionComponent.getAlertTimingPayload();
      ...
      if (this.selectedApplyOnPeriod == 'C') {
        urgencylevelStartDate = Util.convertDateToUtc(
          this.setStartEndDateTime(
            this.alertForm.controls.fromDate.value,
            this.alertForm.controls.fromTimeRange.value,
            'start'
          )
        );
        urgencylevelEndDate = Util.convertDateToUtc(
          this.setStartEndDateTime(
            this.alertForm.controls.toDate.value,
            this.alertForm.controls.toTimeRange.value,
            'end'
          )
        );
      } else {
      ...
      if (this.openAdvancedFilter == true) {
        let alertAdvancedPayload =
          this.alertAdvancedComponent.getAdvancedFilterAlertPayload();
        ...
        if (this.alertFilterRefs.length > 0) {
          this.alertFilterRefs = alertAdvancedPayload[
            'advancedAlertPayload'
      ...
      if (
        this.isNotifyEmailValid &&
        this.isAdvancedAlertPayload &&
        this.isEnteringZone &&
        this.isValidityCalender &&
        this.isFiltersDetailsValidate
      ) {
        ...
        if (this.actionType == 'create' || this.actionType == 'duplicate') {
          urgenyLevelObj = {
            ...
            thresholdValue: '0',
            unitType: 'N',
            dayType: [false, false, false, false, false, false, false],
        ...
        if (
          ...
          if (
            ...
            //Hours of Service
            alertTimingRefHoursOfService =
              this.periodSelectionComponent.getAlertTimingPayload();
            if (this.actionType == 'edit') {
              alertTimingRefHoursOfService.forEach((element) => {
                element['refId'] =
                  this.selectedRowData.alertUrgencyLevelRefs[0].id;
              });
            }
          ...
          if (
            ...
            if (this.selectedPOI.selected.length > 0) {
              if (
                ...
                this.selectedPOI.selected.forEach((element) => {
                  let poiLandmarkRefArr =
                    this.selectedRowData.alertLandmarkRefs.filter(
                      (item) => item.refId == element.id
                    );
                  let tempObj = {
                    landmarkType: 'P',
                    refId: element.id,
                    ...
                    unitType: 'N',
                    id:
                      poiLandmarkRefArr.length > 0
                        ? poiLandmarkRefArr[0].id
                        : 0,
                    alertId: this.selectedRowData.id,
              ...
              if (
                ...
                this.selectedGeofence.selected.forEach((element) => {
                  ...
                  let tempObj = {
                    ...
                    unitType: 'N',
                    id:
                      geofenceLandmarkRefArr.length > 0
                        ? geofenceLandmarkRefArr[0].id
                        : 0,
                    alertId: this.selectedRowData.id,
                    state: element.state == 'Active' ? 'A' : 'I',
                  };
            ...
            if (this.selectedGroup.selected.length > 0) {
              if (
                ...
                this.selectedGroup.selected.forEach((element) => {
                  let groupLandmarkRefArr =
                    this.selectedRowData.alertLandmarkRefs.filter(
                      (item) =>
                        item.refId == element.id && item.landmarkType == 'G'
                    );
                  let tempObj = {
                    landmarkType: 'G',
                    refId: element.id,
                    ...
                    unitType: 'N',
                    id:
                      groupLandmarkRefArr.length > 0
                        ? groupLandmarkRefArr[0].id
                        : 0,
                    alertId: this.selectedRowData.id,
                    state: 'A',
                  };
              ...
              if (
                ...
                this.selectedCorridor.selected.forEach((element) => {
                  ...
                  let tempObj = {
                    ...
                    unitType: 'N',
                    id:
                      corridorLandmarkRefArr.length > 0
                        ? corridorLandmarkRefArr[0].id
                        : 0,
                    alertId: this.selectedRowData.id,
                    state: element.state,
                  };
          ...
          if (this.isCriticalLevelSelected) {
            ...
            if (this.actionType == 'create' || this.actionType == 'duplicate') {
              ...
              let urgencyLevelRefArr =
                this.selectedRowData.alertUrgencyLevelRefs.filter(
                  (item) => item.urgencyLevelType == 'C'
                );
              criticalUrgenyLevelObj = {
                urgencyLevelType: 'C',
                thresholdValue: this.criticalThreshold.toString(),
                ...
                urgencylevelEndDate: urgencylevelEndDate,
                id:
                  urgencyLevelRefArr.length > 0 ? urgencyLevelRefArr[0].id : 0,
                alertId: this.selectedRowData.id,
                alertFilterRefs: this.alertFilterRefs,
                alertTimingDetails: alertTimingRefHoursOfService,
              };
        ...
        if (this.actionType == 'create' || this.actionType == 'duplicate') {
          ...
          this.showLoadingIndicator = true;
          this.alertService.createAlert(createAlertObjData).subscribe(
            (data) => {
              if (data) {
                this.alertCreatedMsg = this.getAlertCreatedMessage();
                let emitObj = {
                  actionFlag: false,
                  successMsg: this.alertCreatedMsg,
                };
                this.backToPage.emit(emitObj);
              }
              this.showLoadingIndicator = false;
            ...
            (error) => {
              ...
              if (
                error.status == 409 &&
                error.error == 'Duplicate alert name'
              ) {
                ...
                error.status == 409 &&
                error.error.includes('Duplicate notification recipient label')
              ) {
          ...
          this.showLoadingIndicator = true;
          this.alertService.updateAlert(editAlertObjData).subscribe(
            (data) => {
              if (data) {
                this.alertCreatedMsg = this.getAlertCreatedMessage();
                let emitObj = {
            ...
            (error) => {
              this.showLoadingIndicator = false;
              if (error.status == 409) this.isDuplicateAlert = true;
            }
          );
        }
      }
  ...
  private scrollToVehicleInvalidControl() {
    ...
    if (this.selectedApplyOn == 'G') {
      if (
        this.alertForm.controls.vehicleGroup.value == '' ||
        this.alertForm.controls.vehicleGroup.value == 'ALL'
      ) {
      ...
      if (
        this.alertForm.controls.vehicle.value == '' ||
        this.isUnsubscribedVehicle
      ) {
        this.alertForm.get('vehicle').setValue('');
        invalidControl = this.el.nativeElement.querySelector(
  ...
  private scrollToFiltersDetailsInvalidControl() {
    ...
    if (
      ...
      if (this.filterDetailsCheck) {
        if (
          !this.isWarningLevelSelected &&
          !this.isAdvisoryLevelSelected &&
          !this.isCriticalLevelSelected
        ) {
          this.isExpandedOpen = true;
          ...
          this.isCriticalLevelSelected &&
          (this.alertForm.get('criticalLevelThreshold').value == '' ||
            this.alertForm.get('criticalLevelThreshold').value == null)
        ) {
          this.alertForm.get('criticalLevelThreshold').setValue('');
          invalidControl = this.el.nativeElement.querySelector(
            '[formcontrolname="' + 'criticalLevelThreshold' + '"]'
          );
          this.filterDetailsErrorMsg =
            'Critical level threshold cannot be empty';
        } else if (
          this.isWarningLevelSelected &&
          !this.isAdvisoryLevelSelected &&
          !this.isCriticalLevelSelected &&
          (this.alertForm.get('warningLevelThreshold').value == '' ||
            this.alertForm.get('warningLevelThreshold').value == null)
        ) {
          this.alertForm.get('warningLevelThreshold').setValue('');
          invalidControl = this.el.nativeElement.querySelector(
            '[formcontrolname="' + 'warningLevelThreshold' + '"]'
          );
          this.filterDetailsErrorMsg =
            'Warning level threshold cannot be empty';
        } else if (
          !this.isWarningLevelSelected &&
          this.isAdvisoryLevelSelected &&
          !this.isCriticalLevelSelected &&
          (this.alertForm.get('advisoryLevelThreshold').value == '' ||
            this.alertForm.get('advisoryLevelThreshold').value == null)
        ) {
          this.alertForm.get('advisoryLevelThreshold').setValue('');
          invalidControl = this.el.nativeElement.querySelector(
            '[formcontrolname="' + 'advisoryLevelThreshold' + '"]'
          );
          this.filterDetailsErrorMsg =
            'Advisory level threshold cannot be empty';
        }
        ...
        (this.isWarningLevelSelected &&
          this.isCriticalLevelSelected &&
          Number(this.alertForm.get('warningLevelThreshold').value) >=
            Number(this.alertForm.get('criticalLevelThreshold').value))
      ) {
        invalidControl = this.el.nativeElement.querySelector(
          '[formcontrolname="' + 'warningLevelThreshold' + '"]'
        );
        this.filterDetailsErrorMsg =
          this.translationData.lblWarningValueShouldbeLess;
      } else if (
        (this.isAdvisoryLevelSelected &&
          this.alertForm.get('advisoryLevelThreshold').value == '') ||
        (this.isAdvisoryLevelSelected &&
          this.isWarningLevelSelected &&
          Number(this.alertForm.get('advisoryLevelThreshold').value) >=
            Number(this.alertForm.get('warningLevelThreshold').value)) ||
        (this.isAdvisoryLevelSelected &&
          this.isCriticalLevelSelected &&
          Number(this.alertForm.get('advisoryLevelThreshold').value) >=
        ...
        this.filterDetailsErrorMsg =
          this.translationData.lblAdvisoryValueShouldBeLess;
      }
    } else {
      invalidControl = null;
  ...
  masterToggleForPOI() {
    this.isAllSelectedForPOI()
      ? this.selectedPOI.clear()
      : this.poiDataSource.data.forEach((row) => this.selectedPOI.select(row));
  }
  ...
  checkboxLabelForPOI(row?: any): string {
    if (row) return `${this.isAllSelectedForPOI() ? 'select' : 'deselect'} all`;
    else
      return `${this.selectedPOI.isSelected(row) ? 'deselect' : 'select'} row`;
  }
  ...
  checkboxLabelForGeofence(row?: any): string {
    if (row)
      ...
      return `${
        this.selectedGeofence.isSelected(row) ? 'deselect' : 'select'
      } row`;
  }
  ...
  checkboxLabelForGroup(row?: any): string {
    if (row)
      ...
      return `${
        this.selectedGroup.isSelected(row) ? 'deselect' : 'select'
      } row`;
  }
  ...
  checkboxLabelForCorridor(row?: any): string {
    if (row)
      ...
      return `${
        this.selectedCorridor.isSelected(row) ? 'deselect' : 'select'
      } row`;
  }
  ...
  onChange($event) {
    this.criticalThreshold = Number(
      this.alertForm.controls.criticalLevelThreshold.value
    );
    this.warningThreshold = Number(
      this.alertForm.controls.warningLevelThreshold.value
    );
    this.advisoryThreshold = Number(
      this.alertForm.controls.advisoryLevelThreshold.value
    );
  }
  ...
  filterVehicleGroups(vehicleSearch) {
    ...
    this.filteredVehicleGroups.next(
      this.vehicleGroupList.filter(
        (item) =>
          item.vehicleGroupName.toLowerCase().indexOf(vehicleSearch) > -1
      )
    );
  }
  ...
  filterVehicles(search) {
    ...
    this.filteredVehicles.next(
      this.vehicleByVehGroupList.filter(
        (item) => item.vin.toLowerCase().indexOf(search) > -1
      )
    );
  }
