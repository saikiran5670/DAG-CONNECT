...
export class NotificationAdvancedFilterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('contextOrgId')) {
      this.organizationId = localStorage.getItem('contextOrgId')
    ...
    this.days = [
      this.translationData.lblSunday,
      this.translationData.lblMonday,
      this.translationData.lblTuesday,
      this.translationData.lblWednesday,
      this.translationData.lblThursday,
      this.translationData.lblFriday,
      this.translationData.lblSaturday,
    ];
    ...
    if (
      (this.actionType == 'edit' || this.actionType == 'duplicate') &&
      ...
      if (this.selectedRowData.notifications[0].validityType == 'C') {
        this.selectedRowData.notifications[0].alertTimingDetail.forEach(
          (element, index) => {
            element.dayType.forEach((item, index) => {
              if (item == true) {
                let totalTime = this.convertTimeIntoHours(
                  element.startDate,
                  element.endDate
                );
  ...
  setAlertType(alertType: any) {
    ...
    if (
      ...
      this.notificationAdvancedFilterForm
  ...
  initPeriodItems(): FormGroup {
    return this._formBuilder.group({
      daySelection: [''],
      fulldayCustom: [''],
      FormArrayCustomItems: this._formBuilder.array([
        this.initCustomPeriodItems(),
      ]),
      id: [],
    });
  }
  ...
  addCustomPeriod(periodIndex, totalTime?, isButtonClicked?) {
    if (this.actionType == 'create') {
      ...
      if (isButtonClicked) {
        ...
        this.customPeriods(periodIndex).push(
          this.setCustomPeriodItems(totalTime[0], totalTime[1])
        );
      }
    }
  }
  ...
  weekDays(): FormArray {
    return this.notificationAdvancedFilterForm.get(
      'FormArrayItems'
    ) as FormArray;
  }
  ...
  customPeriods(periodIndex: number): FormArray {
    return this.weekDays()
      .at(periodIndex)
      .get('FormArrayCustomItems') as FormArray;
  }
  ...
  setDefaultValues() {
    ...
    this.notificationAdvancedFilterForm
      .get('notificationFrequency')
      .setValue(this.selectedRowData.notifications[0].frequencyType);
    this.notificationAdvancedFilterForm
      .get('validityAlwaysCustom')
      .setValue(this.selectedRowData.notifications[0].validityType);
  ...
  getNotificationAdvancedFilter() {
    ...
    let tempObj: any;
    if (
      this.notificationAdvancedFilterForm.controls.validityAlwaysCustom.value ==
      'C'
    ) {
      let weekDaysList = this.weekDays().controls.filter(
        (i) => i['controls'].daySelection.value != ''
      );
    ...
    return {
      frequencyType:
        this.notificationAdvancedFilterForm.controls.notificationFrequency
          .value,
      validityType:
        this.notificationAdvancedFilterForm.controls.validityAlwaysCustom.value,
