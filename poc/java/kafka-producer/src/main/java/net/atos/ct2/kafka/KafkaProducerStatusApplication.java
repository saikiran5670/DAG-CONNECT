...
public class KafkaProducerStatusApplication {
        ...
        Files.readAllLines(Paths.get(jsonFile))
                ...
                .map(status -> {
                   /* long timeMillis = System.currentTimeMillis();
                    ...
                    status.setEvtDateTime(Utils.convertMillisecondToDateTime(timeMillis));
                    status.setReceivedTimestamp(timeMillis);
                    status.setDocument(document);
                    long sleepTime = Long.valueOf(env.getProperty("status.set.system.event.time.sleep","1000"));
                    try {
