...
{
    public class CustomerDataCdcRepository : ICustomerDataCdcRepository
    {
        private readonly IDataAccess _dataAccess;
        private readonly IDataMartDataAccess _dataMartdataAccess;

        public CustomerDataCdcRepository(IDataAccess dataAccess, IDataMartDataAccess dataMartdataAccess)
        {
        ...
        public async Task<List<VehicleAlertRef>> GetVehiclesAndAlertFromCustomerDataConfiguration(int subscriptionId)
        {
            try
            {
                var parameter = new DynamicParameters();
                parameter.Add("@subscriptionId", subscriptionId);
                string query = @"with cte_subscribe_alert
                                 AS (
                                   select distinct 
                                    ale.id as alertid
                            ...
                            on sub.package_id=pac.id and pac.state='A'
                            ...
                            where sub.id =@subscriptionId and sub.organization_id in(ale.organization_id)
                            and fea.id= enutra.feature_id
                            ...
                            --select * from cte_subscribe_alert
        ...
        public async Task<IEnumerable<int>> GetAlertFeatureIds(int orgnisationId)
        {
            try
            {
                var parameter = new DynamicParameters();
                ...
                string query = @"SELECT distinct f.id 
		                    ...
		                    INNER JOIN master.Subscription s ON s.package_id = pkg.id 
