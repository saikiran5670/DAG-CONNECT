...
{
    public class LandmarkAlertCdcRepository : ILandmarkAlertCdcRepository
    {
        private readonly IDataAccess _dataAccess;
        private readonly IDataMartDataAccess _dataMartdataAccess;

        public LandmarkAlertCdcRepository(IDataAccess dataAccess, IDataMartDataAccess dataMartdataAccess)
        {
        ...
        public async Task<List<int>> GetAlertsbyLandmarkId(int landmarkid, string landmarktype)
        {
            try
            {
                var parameter = new DynamicParameters();
                ...
                if (landmarktype == "")
                {
                    query = @"select distinct alert_id
                          ...
                          where ref_id= @landmark_id and state =@State";
                ...
                var data = await _dataAccess.QueryAsync<int>(query, parameter);
                //
                return data.AsList();
            }
            catch (Exception)
        ...
        public async Task<List<VehicleAlertRef>> GetVehiclesFromAlertConfiguration(List<int> alertId)
        {
            try
            {
                var parameter = new DynamicParameters();
                ...
                string query = @"with cte_alert_vehicle_groupanddynamic
                            ...
                            on cte.vehicle_group_id = grp.id and grp.object_type='V' and cte.id= ANY(@alertid)
