    ...
    public partial class ReportRepository : IReportRepository
    {
                                                                                              ...
                                                                                              int organizationId, int contextOrgId, int[] featureId)
        {
            try
            {
                ...
                var parameter = new DynamicParameters();
                string strReportIds = string.Empty;
                IEnumerable<int> ReportIds = await GetSubReportIds(featureId);
                if (ReportIds.Count() > 0)
                {
                    parameter.Add("@report_id", ReportIds.ToList());

                    strReportIds = "ANY(@report_id)";
                }
                else
                {
                    parameter.Add("@report_id", reportId);
                ...
                string queryUpdated = string.Format(query, strReportIds);
                #endregion

                return await _dataAccess.QueryAsync<ReportUserPreference>(queryUpdated, parameter);
            }
            catch (Exception)
            {
                throw;
        ...
        private async Task<IEnumerable<int>> GetSubReportIds(int[] featureIds)
        {
            try
            {
                var parameter = new DynamicParameters();
        ...
        public async Task<dynamic> GetKPIInfo(EcoScoreDataServiceRequest request)
        {
            ...
            try
            {
                ...
                string query =
                    ...
                    AverageDistancePerDay as 
                    (
	                    select eco.driver1_id, eco.aggregation_type, CAST(SUM(eco.trip_distance) AS DOUBLE PRECISION) as AverageDistancePerDay_Total, CAST(COUNT(DISTINCT CAST(date_trunc('DAY', to_timestamp(eco.end_time/1000) AT TIME ZONE @UserPrefTimeZone) AS TEXT)) AS DOUBLE PRECISION) as AverageDistancePerDay_Count
	                    FROM ecoscorequery eco
	                    GROUP BY eco.driver1_id, eco.aggregation_type
                    ),
                    EcoScore as
                    ...
                    FuelConsumption as 
                    (
	                    SELECT eco.driver1_id, eco.aggregation_type, (CAST(SUM (eco.used_fuel)AS DOUBLE PRECISION)) as FuelConsumption_Total, SUM(eco.trip_distance) as FuelConsumption_Count
	                    FROM ecoscorequery eco
	                    GROUP BY eco.driver1_id, eco.aggregation_type
                    ),
                    CruiseControlUsage as 
                    ...
                    Left join AverageDistancePerDay avgdperday on avgdperday.driver1_id = noveh.driver1_id and noveh.aggregation_type = avgdperday.aggregation_type
        ...
        public async Task<dynamic> GetChartInfo(EcoScoreDataServiceRequest request)
        {
            ...
            try
            {
                ...
                string query =
                    @"WITH 
                    ecoscorequery as (
	                    SELECT eco.driver1_id, eco.trip_distance, eco.dpa_Braking_score, eco.dpa_Braking_count, 
