    ...
    public class GroupRepository : IGroupRepository
    {
        ...
        public async Task<IEnumerable<Group>> GetVehicleGroupWithVehCount(GroupFilter groupFilter)
        {
            try
            {
                var parameter = new DynamicParameters();
                List<Group> groupList = new List<Group>();
                var query = @"select id,object_type,group_type,argument,function_enum,organization_id,ref_id,name,description,created_at from master.group where 1=1 and (function_enum <> 'M' or function_enum is null) ";
