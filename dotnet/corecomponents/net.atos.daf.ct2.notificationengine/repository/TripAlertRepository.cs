...
namespace net.atos.daf.ct2.notificationengine.repository
{
    public class TripAlertRepository : ITripAlertRepository
    {
        ...
        public TripAlertRepository(IDataAccess dataAccess, IDataMartDataAccess dataMartdataAccess)
        {
        ...
        public async Task<TripAlert> CreateTripAlert(TripAlert tripAlert)
        {
            try
            {
                if (tripAlert.Alertid > 0) //need to revisit, it only for testing 
                {
                    ...
                    var id = await _dataMartdataAccess.ExecuteScalarAsync<int>(query, parameter);
                    tripAlert.Id = id;
                }
            }
            catch (Exception)
