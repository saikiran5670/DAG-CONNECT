using System;
using System.Threading.Tasks;
using Microsoft.Extensions.Options;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using net.atos.daf.ct2.identity.entity;

namespace net.atos.daf.ct2.identity.test
{
    [TestClass]
    public class AccountAuthenticatorTest
    {
        private readonly IAccountAuthenticator _autheticator;
        public AccountAuthenticatorTest()
        {
            var idenityconfiguration = new IdentityJsonConfiguration()
            {
                Realm = "DAFConnect",
                BaseUrl = "http://104.45.77.70:8080",
                AuthUrl = "/auth/realms/{{realm}}/protocol/openid-connect/token",
                UserMgmUrl = "/auth/admin/realms/{{realm}}/users",
                AuthClientId = "admin-cli",
                AuthClientSecret = "8b7a26b2-ebd7-4dff-830d-eb1f884d85bc",
                UserMgmClientId = "DAF-Admin",
                UserMgmClientSecret = "8b7a26b2-ebd7-4dff-830d-eb1f884d85bc",
                Issuer = "Me",
                Audience = "You",
                RsaPrivateKey = "",
                RsaPublicKey = ""
            };

            IOptions<IdentityJsonConfiguration> setting = Options.Create(idenityconfiguration);
            _autheticator = new AccountAuthenticator(setting);

        }
        [TestCategory("Unit-Test-Case")]
        [Description("Test authenticate user with correct username and password- Login")]
        [TestMethod]
        public async Task AuthenticateUserTest()
        {
            Identity user = new Identity();
            user.UserName = "testuser5@atos.net";
            user.Password = "123456";

            Response result = await _autheticator.AccessToken(user);
            Assert.IsNotNull(result);
            Assert.IsTrue(result.StatusCode == System.Net.HttpStatusCode.OK);
        }

        [TestCategory("Unit-Test-Case")]
        [Description("Test authenticate user with wrong username and password- Login")]
        [TestMethod]
        public async Task UnauthorizedUserTest()
        {
            Identity user = new Identity();
            user.UserName = "testuser500@atos.net";
            user.Password = "1234567";

            Response result = await _autheticator.AccessToken(user);
            string message = Convert.ToString(result.Result);
            Assert.IsNotNull(result);
            Assert.IsTrue(result.StatusCode == System.Net.HttpStatusCode.Unauthorized);
            Assert.IsTrue(message.Contains("Invalid user credentials"));

        }
    }
}
