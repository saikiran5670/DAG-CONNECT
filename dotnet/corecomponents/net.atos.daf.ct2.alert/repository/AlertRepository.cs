    ...
    public class AlertRepository : IAlertRepository
    {
        ...
        private async Task<int> CreateAlertUrgencyLevelRef(AlertUrgencyLevelRef urgencylevel)
        {
            try
            {
                ...
                parameterurgencylevelref.Add("@day_type", Convert.ToChar(urgencylevel.DayType));
                ...
                string queryLandmarkref = @"INSERT INTO master.alerturgencylevelref(alert_id, urgency_level_type, threshold_value, unit_type, day_type, period_type, urgencylevel_start_date, urgencylevel_end_date, state, created_at)
                ...
                int id= await dataAccess.ExecuteScalarAsync<int>(queryLandmarkref, parameterurgencylevelref);
                return id;
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> CreateAlertFilterRef(AlertFilterRef alertfilter)
        {
            try
            {
                ...
                parameteralertfilterref.Add("@day_type", alertfilter.DayType);
                ...
                string queryLandmarkref = @"INSERT INTO master.alertfilterref(alert_id, alert_urgency_level_id, filter_type, threshold_value, unit_type, landmark_type, ref_id, position_type, day_type, period_type, filter_start_date, filter_end_date, state, created_at)
                ...
                int id = await dataAccess.ExecuteScalarAsync<int>(queryLandmarkref, parameteralertfilterref);
                return id;
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> CreateNotification(Notification notification)
        {
            try
            {
                ...
                string queryLandmarkref = @"INSERT INTO master.alertfilterref(alert_id, alert_urgency_level_id, filter_type, threshold_value, unit_type, landmark_type, ref_id, position_type, day_type, period_type, filter_start_date, filter_end_date, state, created_at)
	                                    VALUES (@alert_id, @alert_urgency_level_id, @filter_type, @threshold_value, @unit_type, @landmark_type, @ref_id, @position_type, @day_type, @period_type, @filter_start_date, @filter_end_date, @state, @created_at) RETURNING id";
                int id = await dataAccess.ExecuteScalarAsync<int>(queryLandmarkref, parameternotification);
                return id;
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> CreateNotificationAvailabilityPeriod(NotificationAvailabilityPeriod availabilityperiod)
        {
            try
            {
                ...
                string queryLandmarkref = @"INSERT INTO master.notificationavailabilityperiod(notification_id, availability_period_type, period_type, start_time, end_time, state, created_at)
	                                    VALUES (@notification_id, @availability_period_type, @period_type, @start_time, @end_time, @state, @created_at) RETURNING id";
                var id = await dataAccess.ExecuteScalarAsync<int>(queryLandmarkref, parameteravailabilityperiod);
                return id;
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> CreateNotificationLimit(NotificationLimit limit)
        {
            try
            {
                ...
                string queryLandmarkref = @"INSERT INTO master.notificationlimit(notification_id, notification_mode_type, max_limit, notification_period_type, period_limit, state, created_at)
                ...
                var id = await dataAccess.ExecuteScalarAsync<int>(queryLandmarkref, parameterlimit);
                return id;
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> CreateNotificationrecipient(NotificationRecipient recipient)
        {
            try
            {
                ...
                string queryLandmarkref = @"INSERT INTO master.notificationrecipient(notification_id, recipient_label, account_group_id, notification_mode_type, phone_no, sms, email_id, email_sub, email_text, ws_url, ws_type, ws_text, ws_login, ws_password, state, created_at)
                ...
                var id = await dataAccess.ExecuteScalarAsync<int>(queryLandmarkref, parameterrecipient);
                return id;
            }
            catch (Exception ex)
            {
