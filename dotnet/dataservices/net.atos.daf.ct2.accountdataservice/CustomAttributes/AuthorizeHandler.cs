          ...
          AuthorizationHandler<AuthorizeRequirement>
    {
            ...
            AuthorizationHandlerContext context, AuthorizeRequirement requirement)
        {
            ...
            if (emailClaim != null && !string.IsNullOrEmpty(emailClaim.Value))
            {
                ...
                _logger.Info($"[AccountDataService] Email claim received: {emailAddress}");
            }
            else
            {
                context.Fail();
            ...
            try
            {
                ...
                _logger.Info($"[AccountDataService] Is user authorized: {isExists}");
                if (isExists)
            ...
            {
                _logger.Error("[AccountDataService] Error occurred while authorizing the request", ex);
                context.Fail();
                return;
            }
        }
