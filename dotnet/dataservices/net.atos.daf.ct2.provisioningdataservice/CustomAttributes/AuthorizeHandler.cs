          ...
          AuthorizationHandler<AuthorizeRequirement>
    {
            ...
            AuthorizationHandlerContext context, AuthorizeRequirement requirement)
        {
            ...
            if (emailClaim != null && !string.IsNullOrEmpty(emailClaim.Value))
            {
                ...
                _logger.Info($"[ProvisioningDataService] Email claim received: {emailAddress}");
            }
            else
            {
                context.Fail();
            ...
            try
            {
                ...
                _logger.Info($"[ProvisioningDataService] Is user authorized: {isExists}");
                if (isExists)
            ...
            {
                _logger.Error("[ProvisioningDataService] Error occurred while authorizing the request", ex);
                context.Fail();
                return;
            }
        }
