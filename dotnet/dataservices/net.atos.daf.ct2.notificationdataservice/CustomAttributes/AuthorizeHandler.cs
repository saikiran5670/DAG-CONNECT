          ...
          AuthorizationHandler<AuthorizeRequirement>
    {
            ...
            AuthorizationHandlerContext context, AuthorizeRequirement requirement)
        {
            ...
            if (emailClaim != null && !string.IsNullOrEmpty(emailClaim.Value))
            {
                ...
                _logger.Info($"[NotificationDataService] Email claim received: {emailAddress}");
            }
            else
            {
                context.Fail();
            ...
            try
            {
                ...
                _logger.Info($"[NotificationDataService] Is user authorized: {isExists}");
                if (isExists)
            ...
            {
                _logger.Error("[NotificationDataService] Error occurred while authorizing the request", ex);
                context.Fail();
                return;
            }
        }
