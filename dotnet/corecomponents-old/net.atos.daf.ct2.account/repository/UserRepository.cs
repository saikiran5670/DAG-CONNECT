...
namespace net.atos.daf.ct2.user.repository
{
    public class UserRepository:IUserRepository
    {
        ...
        public UserRepository(IDataAccess _dataAccess) 
        {
       ...
       public async Task<int>  AddUser(UserDetails user)
       {
             int UserId=0;
             int userorg=0;
       ...
       public async Task<int> AddUserorg(UserDetails user)
       {
           try
           {
            var parameter = new DynamicParameters();
       ...
       public async Task<int> AddUserPreferences(UserGenralPreferences userpreferences)
       {
           try
           {
            var parameter = new DynamicParameters();
       ...
       public async Task<IEnumerable<User>> GetUserDetails(int userid)
       {
                 var parameter = new DynamicParameters();
                 parameter.Add("@userid", userid);                          
                 return await dataAccess.QueryAsync<User>("Select * from dafconnectmaster.user where userid=@userid ", parameter);     
       ...
       public async Task<IEnumerable<User>> GetUsers(int UsertypeID,bool IsActive)
       {
            try
            {
                 var parameter = new DynamicParameters();
                 ...
                 var resultGetUsers = await dataAccess.QueryAsync<User>("Select * from dafconnectmaster.user where usertypeid=@usertypeid  and isactive=@isActive", parameter);                 
       ...
       public async Task<bool> CheckEmailExist(string emailid)
       {
            try
            {
                 ...
                 var resultGetUsers = await dataAccess.QueryAsync<User>("Select * from dafconnectmaster.user where emailid=@emailid", parameter);                 
       ...
       public async Task<int> AddUserRoles(UserRoleMapping userRoleMapping)
       {
           try
           {
            var parameter = new DynamicParameters();
       ...
       public async Task<IEnumerable<UserRoleMapping>> GetUserRoles(int Userorgid,bool IsActive)
       {
            try
            {
                 var parameter = new DynamicParameters();
