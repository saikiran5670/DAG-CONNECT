...
public class IndexBasedAlertFunctions implements Serializable {
	...
	public static AlertLambdaExecutor<Message, Target> hoursOfServiceFun = (Message s) -> {
		...
		try {
			if (Objects.nonNull(index) && index.getDocument().getVWheelBasedSpeed() <= 0L
					&& index.getDocument().getVEngineSpeed() <= 0L)
				return Target.builder().metaData(s.getMetaData()).payload(s.getPayload()).alert(Optional.empty())
						.build();
					...
					if (schema.getUrgencyLevelType().equalsIgnoreCase(priority)) {
						...
						if (currentDayOfWeek.equalsIgnoreCase(dayOfWeekFromDbArr)) {
							...
							if (schema.getPeriodType().equalsIgnoreCase("C")) {
								...
								if (schema.getStartTime() <= currentTimeInSecond
										&& schema.getEndTime() > currentTimeInSecond) {
									return getTarget(index, schema, convertDateToMillis(index.getEvtDateTime()));
								}
							}
						}
				...
				for (AlertUrgencyLevelRefSchema schema : urgencyLevelRefSchemas) {
					...
					if (schema.getUrgencyLevelType().equalsIgnoreCase(priority)) {
						...
						if (index.getVDist() > Double.valueOf(schema.getThresholdValue())) {
							...
							logger.info("alert found excessiveAverageSpeed ::type {} , threshold {} , index {}",
									schema.getAlertType(), schema.getThresholdValue(), index);
							return getTarget(index, schema, convertDateToMillis(index.getEvtDateTime()));
						}
					}
				}
	...
	private static Target getTarget(Index index, AlertUrgencyLevelRefSchema urgency, Object valueAtAlertTime) {
		return Target.builder().alert(Optional.of(Alert.builder()
				.tripid(index.getDocument() != null ? index.getDocument().getTripID() : "").vin(index.getVin())
				...
				.valueAtAlertTime("" + valueAtAlertTime).urgencyLevelType(urgency.getUrgencyLevelType()).build()))
				.build();
	}
}
